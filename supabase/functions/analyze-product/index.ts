...
function buildSuggestPrompt(npList) {
  ...
  return `你是資深預防保健顧問。根據第一段競品分析，用「三大保養面向」組合最佳搭配建議。

## 可用產品資料（僅供你內部理解產品與競品的對應關係；其中的產品名稱、品牌、商標一律不可寫進輸出）：
${products}

## ★★ 默契骨架（最重要，所有建議都以此為主軸）
保養分三大面向，缺一不可。每個面向對應一個【固定代號】，必須【逐字一致】使用——這是業務員與客戶之間的默契：客戶看到的是中性成分概念，業務員一看代號就秒懂是哪樣產品。
...
## 輸出規則
1. 先點出競品落在哪個保養面向、而且「只做了一半」，用該面向的固定代號＋成分規格數據正面對決，講清楚為何比這隻競品強（攻擊不必客氣）
2. 拉高視角：三大保養面向缺一不可，用固定代號帶出另外兩塊補缺口
  ...
  try {
    ...
    if (payload.type === 'product') {
      ...
      try {
        ...
        const { data } = await sb.from('np_products').select('*').eq('status', 'active').or(`category.eq.${cat || 'none'},is_star.eq.true`).order('sort_order');
        npList = data || [];
      } catch (_e) { npList = []; }
